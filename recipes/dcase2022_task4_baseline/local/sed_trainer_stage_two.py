...
class SEDTask4(pl.LightningModule):
    ...
    def __init__(
        ...
        # self.supervised_loss_mtl = AsymmetricalFocalLoss(self.hparams["training"]["afl_loss"][0], self.hparams["training"]["afl_loss"][1])
        self.supervised_loss_mtl = torch.nn.BCELoss()
        self.supervised_loss = torch.nn.BCELoss()
        #self.supervised_loss_mtl = nn.CrossEntropyLoss()
    ...
    def training_step(self, batch, batch_indx):
        ...
        audio, labels, padded_indxs, labels_mtl = batch
        ...
        labels_weak_mtl = (torch.sum(labels_mtl[weak_mask], -1) > 0).float()
        #Frame Shift
        features, labels, labels_mtl = frame_shift(features, labels, labels_mtl, self.hparams["training"]["net_pooling"])
        
        mixup_type = self.hparams["training"].get("mixup")
        if mixup_type is not None and 0.5 > random.random():
            features[weak_mask], labels_weak, labels_weak_mtl = mixup(
                features[weak_mask], labels_weak, labels_weak_mtl, mixup_label_type=mixup_type
            )
            features[strong_mask], labels[strong_mask], labels_mtl[strong_mask]= mixup(
                features[strong_mask], labels[strong_mask], labels_mtl[strong_mask], mixup_label_type=mixup_type
            )
        
        # Time masking
        features[strong_mask], labels[strong_mask], labels_mtl[strong_mask] = time_mask(features[strong_mask], labels[strong_mask], labels_mtl[strong_mask], self.hparams["training"]["net_pooling"], mask_ratios=self.hparams["training"]["time_mask_ratios"])
        ...
        # sed student forward
        strong_preds_student, weak_preds_student, strong_preds_student_mtl,  weak_preds_student_mtl= self.detect(
        ...
        loss_strong_mtl = self.supervised_loss_mtl(
            strong_preds_student_mtl[strong_mask], labels_mtl[strong_mask]
        )
        # supervised loss on weakly labelled
        loss_weak = self.supervised_loss(weak_preds_student[weak_mask], labels_weak)
        loss_weak_mtl = self.supervised_loss_mtl(weak_preds_student_mtl[weak_mask], labels_weak_mtl)
        alpha, beta= self.l_param()
        # total supervised loss
        tot_loss_supervised = alpha*(loss_strong + loss_weak) + (1-alpha)*(loss_weak_mtl + loss_strong_mtl)
        
        mixed_input_weak, target_a_weak, target_b_weak, lam_weak = mixup_data_sup(features[weak_mask], labels_weak, mixup_sup_alpha)
        _, output_mixed_weak, _, _ =  self.detect(mixed_input_weak, self.sed_student)
        ...
        output_mixed_strong, _, _, _ = self.detect(mixed_input_strong, self.sed_student)
        ...
        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher, strong_preds_teacher_mtl, weak_preds_teacher_mtl = self.detect(
        ...
        strong_self_sup_loss_mtl = self.selfsup_loss(
            strong_preds_student_mtl, strong_preds_teacher_mtl.detach()
        )
        weak_self_sup_loss_mtl = self.selfsup_loss(
            weak_preds_student_mtl, weak_preds_teacher_mtl.detach()
        )
        tot_self_loss = (beta*(strong_self_sup_loss + weak_self_sup_loss) + (1-beta)*(strong_self_sup_loss_mtl + weak_self_sup_loss_mtl)) * weight 
    ...
    def validation_step(self, batch, batch_indx):
        ...
        audio, labels, padded_indxs, _, filenames = batch

        # prediction for student
        mels = self.mel_spec(audio)
        strong_preds_student, weak_preds_student, _, _ = self.detect(mels, self.sed_student, temp = self.hparams["training"]['temp'])
        # prediction for teacher
        strong_preds_teacher, weak_preds_teacher, _, _ = self.detect(mels, self.sed_teacher ,temp = self.hparams["training"]['temp'])
